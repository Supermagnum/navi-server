...
log_info "syncing static published tree under ${NAVI_PUBLISHED_DIR}"
python3 - "$FINAL" "$GEN_ID" "$NAVI_PUBLISHED_DIR" <<'PY'
    ...
    public_regions.append(
        {
            "region_id": rid,
            "generation": gen_id,
            "manifest_url": f"/packs/{rid}/{gen_id}/manifest.json",
            "has_delta_h": bool(region.get("has_delta_h")),
            "bytes": sum(v["bytes"] for v in file_digests.values()),
        }
    )
...
print(f"wrote {published / 'current.json'}")
PY
